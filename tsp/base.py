...
class TSP:
    ...
    @property
    def initial_path_greedy(self):
        route = [0]
        while len(route) < len(self.nodes):
            min_dist = np.inf
            min_point = None
            for running_point in range(len(self.nodes)):
                if running_point not in route:
                    dist = TSP.dist(route[-1], running_point)
                    if dist < min_dist:
                        min_dist = dist
    ...
    def initial_path(self):
        route = np.arange(len(self.nodes))
